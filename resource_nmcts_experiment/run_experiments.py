...
PRESETS = {
    ...
    "highdim_scale_resource": {
        "methods": ["direct_anf", "and_direct_anf", "and_fprm_greedy", "and_fprm_root_beam", "and_fprm_linear_pair", "and_resource_nmcts", "and_profile_resource_nmcts"],
        "random_truth": [],
        "random_anf": [(15, 32)],
        "structured_limit": 0,
        "workers": 6,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    config_dict = {
        ...
        "mcts_simulations": 24 if args.preset in {"smoke", "large_fast", "evidence", "evidence_affine", "ablation_affine", "traditional_small", "traditional_resource", "large_resource", "large_resource_core", "highdim_resource", "highdim_scale_resource"} else (48 if args.preset == "pilot" else (32 if args.preset == "large" else 96)),
        "neural_mcts_simulations": 32 if args.preset in {"smoke", "large_fast", "evidence", "evidence_affine", "ablation_affine", "traditional_small", "traditional_resource", "large_resource", "large_resource_core", "highdim_resource", "highdim_scale_resource"} else (64 if args.preset == "pilot" else (32 if args.preset == "large" else 128)),
        "max_polarities": 32 if args.preset in {"smoke", "pilot"} else (12 if args.preset in {"evidence", "evidence_affine", "ablation_affine", "traditional_small", "traditional_resource", "large_resource", "large_resource_core", "highdim_resource", "highdim_scale_resource"} else (16 if args.preset == "large_fast" else (48 if args.preset == "large" else 384))),
        "gate_mode": "mct",
        "neural_prior_weight": 2.5,
        "task_timeout_s": 300 if args.preset in {"evidence", "evidence_affine", "ablation_affine", "traditional_small", "traditional_resource", "large_resource", "large_resource_core", "highdim_resource", "highdim_scale_resource"} else (120 if args.preset == "pilot" else (180 if args.preset in {"large", "large_fast"} else (300 if args.preset == "main" else 0))),
    }
    methods = cfg["methods"]
    ...
    if args.preset in {"large_resource", "large_resource_core", "highdim_resource", "highdim_scale_resource"}:
        method_rank = {
            "direct_anf": 0,
            "and_direct_anf": 1,
            "and_fprm_greedy": 2,
