...
def load_sshr_modules():
    if not SSHR_DIR.exists():
        raise FileNotFoundError(f"SSHR source directory not found: {SSHR_DIR}")
    sys.path.insert(0, str(SSHR_DIR))
    from paper_data import TABLE_VIII_COUNTS  # type: ignore
    from parallelotope_enum import enumerate_parallelotopes  # type: ignore
...
def write_manifest(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    data = {
        ...
        "source": os.path.relpath(SSHR_DIR, THIS_DIR),
        "rows": len(rows),
        "matching_rows": matching,
        "all_match": matching == len(rows),
        "min_n": min(int(row["n"]) for row in rows),
