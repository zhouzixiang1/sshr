...
if [[ "${RESOURCE_NMCTS_EXTRACTED_PAYLOAD:-0}" != "1" ]]; then
  "$PYTHON_BIN" make_final_upload_plan.py
  "$PYTHON_BIN" analyze_final_upload_plan_tool_audit.py
fi
...
"$PYTHON_BIN" analyze_oracle_resource_citation_audit.py
...
"$PYTHON_BIN" analyze_baseline_fairness_ledger.py
...
"$PYTHON_BIN" analyze_experimental_evidence_ladder.py
"$PYTHON_BIN" analyze_comparison_support_reference_integrity.py
if [[ "${RESOURCE_NMCTS_EXTRACTED_PAYLOAD:-0}" != "1" ]]; then
  "$PYTHON_BIN" reproduce_sshr_table8_candidate_counts.py
fi
...
"$PYTHON_BIN" analyze_bitflip_prior_feature_gate_cv.py
"$PYTHON_BIN" analyze_bitflip_prior_feature_gate_cv_random_control.py
...
if [[ "${RESOURCE_NMCTS_EXTRACTED_PAYLOAD:-0}" != "1" ]]; then
  ...
  "$PYTHON_BIN" analyze_upload_bundle_matrix_audit.py
