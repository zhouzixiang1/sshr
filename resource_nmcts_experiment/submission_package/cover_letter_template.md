...
The main contribution is a search-based oracle synthesis workflow with a broad comparison envelope.  On 177 traditional functions with n <= 6, Pareto-Resource-NMCTS reduces mean T-count and weighted score relative to direct ANF synthesis, and it is evaluated against ESOP, SSHR-H/SSHR-I, ABC/BDD, ROS-style LUT, mockturtle, Caterpillar API, CirKit, RevKit CLI exact-oracle, and RevKit phase/Rz probes.  The manuscript also reports high-dimensional symbolic verification, complete truth-table bridge checks for n = 21-30, learned-control audits, and archive-level reproducibility checks.
