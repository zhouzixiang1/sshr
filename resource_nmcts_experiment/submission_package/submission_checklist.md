...
- Machine snapshot tokens checked by `analyze_public_handoff_freshness_audit.py`:
  PDF pages=52/52; readiness=84 pass + 1 needs author input;
  payload_files=1206; artifact_registry=31 families / 161 raw CSV / 80312 raw rows;
...
- Caterpillar ROS-family source probe: local source/API/build smoke evidence only; it is separate from the bounded API performance probe.
- Caterpillar XAG API performance probe: 531/531 ANF-XAG strategy rows and 177/177 best-of-Caterpillar rows are verified on the traditional `n<=6` functions; this is a bounded implementation-family counterpoint, not a standalone/full ROS baseline.
- ROS reproduction gap audit: ROS-style LUT, line-sensitivity, Caterpillar source-family/API, and executable garbage-pressure rows are proxy or boundary evidence; the official ROS SAT garbage-management component is not reproduced and must not be claimed.
