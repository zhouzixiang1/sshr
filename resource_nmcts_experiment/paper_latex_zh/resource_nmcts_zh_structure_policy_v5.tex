...
这个 guard 的意义是把结构级 policy 相对固定 depth-2 的 score 缺口从 $+0.28\%$ 降为 $0$，但其收益仍然小。由于它需要先评估浅层 screen，再决定是否回退到 depth-2，测试集上相对单独固定 depth-2 screen 反而慢 39.45\%。因此它应被解释为 ``质量安全门控方向已打通''，而不是最终运行时优化已经完成。
