...
\author{中文论文稿 v21：RevKit API baseline 版}
\date{2026年7月8日}
...
下一步最值得投入的方向也很明确。第一，RevKit API baseline 已经显示 phase/Clifford+T netlist 口径很强，因此需要新增一个 phase-oracle 或 Clifford+T-aware 的内部 emitter，或者在论文中严格限定为 X/CNOT/MCT bit-flip oracle 口径。第二，在当前 ROS-style LUT proxy 的基础上继续接入官方 ROS、mockturtle 和 legacy RevKit/CirKit 流程，把比较从 proxy 扩展到更标准的外部流程。第三，large frontier policy 和 cost-aware frontier policy 已经把结构级 AI 从单一质量追求推进到可调质量-时间折中，但还可以继续把辅助线生命周期、T-depth proxy 和 score 统一进多目标 teacher 或 Pareto policy。完成这些以后，论文就更接近“方法贡献”而不是“阶段性项目报告”。
...
\subsection{RevKit API oracle synthesis 对比}

在 toolchain readiness 审计基础上，本文进一步在本地 \texttt{mcts-qoracle} 环境中安装 RevKit Python API，并调用其 \texttt{oracle\_synth} 对 $n\leq6$ 的 177 个完整 truth-table 传统函数逐一合成。该流程不是 ABC proxy：输入为完整 truth table，输出为 RevKit 的 Clifford+T netlist；本文按返回 netlist 统计 T/Tdg 或奇数倍 $R_z(\pi/4)$、CX、qubit-conflict depth、gate 数和额外 qubit 数。这仍不是硬件 mapping，也不是 legacy CirKit CLI，但已经是一个真实 RevKit API baseline。

表~\ref{tab:revkit} 显示，这个 baseline 给出一个必须正视的负面结果。在当前 X/CNOT/\mct{} bit-flip 资源口径下，\method{} 对 ESOP、ABC-LUT proxy 和 direct ANF 有明显优势；但在 RevKit 的 Clifford+T netlist proxy 下，\method{} 相对 RevKit 的 score 为 6/171/0，平均高 751.69\%，T-count 平均高 4060.08\%。这不应被解释为本文方法失败，而应解释为“目标 oracle 表示不同”：RevKit 的 phase/Clifford+T 合成可以非常直接地利用相位多项式结构，而本文当前 emitter 固定在 X/CNOT/\mct{} compute/action/uncompute 路线。因此，投稿前要么把论文主张严格限定在 bit-flip oracle 逻辑层，要么新增 phase-oracle/Clifford+T-aware emitter 与 RevKit 正面对齐。

\begin{table}[H]
\centering
\small
\caption{RevKit Python \texttt{oracle\_synth} baseline。结果来自 177 个 $n\leq6$ 完整 truth-table 函数。}
\label{tab:revkit}
\begin{tabular}{llrrr}
\toprule
比较 & 指标 & 数量 & 胜/负/平 & 平均 $\Delta$ \\
\midrule
\method{} vs RevKit oracle\_synth & score & 177 & 6/171/0 & $+751.69\%$ \\
Pareto-\method{} vs RevKit oracle\_synth & score & 177 & 6/171/0 & $+711.60\%$ \\
FPRM polarity archive vs RevKit oracle\_synth & score & 177 & 4/173/0 & $+774.83\%$ \\
SSHR-H vs RevKit oracle\_synth & CNOT & 177 & 34/141/2 & $+34.23\%$ \\
\method{} vs RevKit oracle\_synth & T-count & 177 & 2/171/4 & $+4060.08\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
RevKit API 给出新的强外部边界 & RevKit Python \texttt{oracle\_synth} 在 177 个传统函数上全部可运行；在 Clifford+T netlist proxy 下，\method{} 相对 RevKit score 为 6/171/0。 & 口径不同：RevKit 更接近 phase/Clifford+T oracle，本文当前 emitter 是 X/CNOT/\mct{} bit-flip 逻辑。 \\
