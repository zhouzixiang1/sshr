...
portfolio 可以找到该受限模型之外的更优候选。本文结果限定在逻辑线路层，不包含
硬件映射、连通性约束或真实设备噪声模型。
\end{abstract}
...
并处理工作比特管理。BDD-based synthesis 则使用决策图描述可逆逻辑，适合作为
大函数的可扩展 baseline。
...
baseline 为 528 行结果，exact FPRM-DP 切片为 72 行结果。
...
更宽的 CNOT-only linear-pair 根层动作集合存在小幅可学习空间，但当前 high-dimensional
neural scorer 尚未利用该信号。
...
高维 stress test 和 exact FPRM-DP 切片。
...
因此，高维部分目前应强调 bounded linear-pair guard 和 portfolio 选择，而不是夸大学习先验。
...
ESOP/ABC/BDD/SSHR 对比、小规模 exact FPRM-DP 切片和 $n=18$ 高维 stress test 上均表现出
低 T-count 与低加权资源优势。当前证据尚不足以支持 CNOT-only 最优或硬件映射优势，但已经
形成一条比单纯复现 SSHR 更适合发展的投稿主线：以资源约束搜索、神经先验和 bounded
高维 guard 为核心的逻辑层量子布尔 oracle 综合。
