...
Depth-frontier policy 学习质量-时间折中 & 相对 fixed depth-2 为 35/0/37、平均 score -2.19\%；相对 all-depth depth$\leq4$ 平均 score +0.97\%，时间 -58.69\%。 & 可作为新增结构级 AI 质量证据。 \\
高维 emitted circuit 语义可信 & 3720/3720 项集级方法行通过 plan 展开和 emitted circuit GF(2) 符号模拟验证。 & 可支撑项集级结果；truth bridge 进一步补强但样本仍小。 \\
...
下一步最直接的提升目标已经从 ``证明 depth-frontier 存在'' 转为 ``把高预算质量进一步压缩成更便宜的策略''。具体来说，应改进 depth-frontier policy 对 depth-4 oracle 的覆盖率，减少当前相对 depth-4 的 0.97\% 平均 score gap；同时引入更强外部工具链对比，尤其是 ROS、mockturtle/ABC 风格映射和 back-end-aware oracle synthesis，使论文不只和内部 baseline 比较。
