...
两组项集级实验合计覆盖 336 个项集和 2352 个方法行。所有方法行均通过两层验证：factor plan 的 \anf{} 展开验证为 2352/2352，emitted X/CNOT/MCT circuit 的 GF(2) 多项式符号模拟验证为 2352/2352，mismatch 总数为 0。这使项集级结果不只是资源估计，但仍应明确它不是完整 \ttable{} simulation。
