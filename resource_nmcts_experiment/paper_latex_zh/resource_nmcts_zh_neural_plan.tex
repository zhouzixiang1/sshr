...
\begin{abstract}
本文整理一个可继续发展为正式投稿的中文方案：将量子布尔函数 oracle 综合建模为 \anf{}/\fprm{} 项集合上的资源约束搜索问题，并以神经动作先验、蒙特卡洛树搜索、linear-pair 因子、baseline-preserving guard 和 Pareto 候选库共同生成逻辑层低资源线路。该方案不依赖 SSHR 的 parallelotope 候选空间；SSHR 只作为 CNOT-oriented small-scale baseline 参与比较。当前最强证据来自三条线：第一，$n\leq6$ 的 177 个传统函数上，\pareto{} 相对 ESOP cube beam 获得 174/0/3 的 score 胜/负/平，平均 score 降低 36.09\%；第二，$n=16$ 的 24 个随机 ANF 函数上，baseline-preserving AI guard 相对 deterministic recursive guard 获得 6/0/18，平均 score 降低 0.05\%，说明神经排序在确定性兜底下可以转化为无 score-loss 小增益；第三，$n=18$ 的 12 个函数上，\method{} 相对 direct ANF 平均 T-count 降低 60.05\%，平均 score 降低 56.99\%。新增 pairwise root-action ranker 将 $n=14$ 高维 learned prior 结果从 1/0/11、平均 score 降低 0.01\% 改进到 2/0/10、平均 score 降低 0.04\%，但运行时间增加 124.91\%，因此当前 AI 证据仍不足以作为目标结束点。本文建议把论文主张限定为逻辑层低 T-count 与低加权资源优势，并把下一阶段重点放在更强的高维神经排序、外部 reversible-toolchain 复现和代表性线路案例上。
\end{abstract}
...
Pairwise root-action ranker & 用同一根状态内动作对训练排序的高维动作模型 & 当前是弱正向诊断，不是最终突破 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Pairwise root-action ranker 的新增结果}

上一阶段的高维 learned prior 结果很弱：在 12 个 $n=14$ 函数上，learned prior 相对 no-prior 的 score 胜/负/平为 1/0/11，平均 score 只降低 0.01\%，运行时间增加约 104.07\%。这说明 immediate-gain 标签没有有效学到高维根动作排序。

新增 pairwise root-action ranker 改用同一根状态内的动作对进行训练，并用 root-teacher 标签鼓励更好动作排在更前。诊断结果见表~\ref{tab:pairwise}。它把 final highdim neural prior 的 score 胜/负/平提高到 2/0/10，平均 score 降低 0.04\%；root-action 诊断中 neural top-4 相对 heuristic top-4 变为 2/1/7，平均 score 降低 0.01\%。这说明 pairwise 训练确实降低了负迁移，但幅度仍小，且 final synthesis 运行时间增加 124.91\%。因此这不是目标结束点，只能写成下一轮高维 AI 改进的起点。

\begin{table}[t]
\centering
\small
\caption{高维 pairwise root-action ranker 的阶段性结果。}
\label{tab:pairwise}
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}Xrrr}
\toprule
比较 & 函数数 & score 胜/负/平 & 平均 $\Delta$ score \\
\midrule
Oracle top-24 vs heuristic top-1 & 10 & 7/0/3 & $-0.43\%$ \\
Oracle top-24 vs heuristic top-4 & 10 & 3/0/7 & $-0.18\%$ \\
Pairwise neural top-4 vs heuristic top-4 & 10 & 2/1/7 & $-0.01\%$ \\
Pairwise learned prior vs no-prior final synthesis & 12 & 2/0/10 & $-0.04\%$ \\
\bottomrule
\end{tabularx}
\end{table}
...
第一，高维神经排序必须从弱正向变成可见增益。pairwise root-action ranker 已经把负迁移压低，但 0.04\% 的 score 改善不足以支撑强 AI 主张。下一步应训练更强的 root-action/value ranker，例如扩大 teacher width、加入 top-k distillation、使用 pairwise/listwise 混合损失，并在 $n=14,16$ 上报告 matched no-prior、heuristic-prior 和 learned-prior 三组对比。
...
本文当前最稳的主线是：\method{} 将量子布尔函数 oracle 综合转化为 ANF/FPRM 项集合上的资源约束搜索，并通过神经先验、MCTS、linear-pair guard 和 portfolio selection 获得逻辑层低 T-count 与低加权 score 优势。传统函数和高维随机 ANF 已经给出可写入论文的结果，pairwise root-action ranker 也提供了更合理的高维 AI 训练方向。与此同时，高维 learned prior 的实际收益仍小，外部 reversible-toolchain 对比和线路案例仍需补强。因此，这份文档应作为下一轮实验和正式英文稿的路线图，而不是最终定稿。
