...
\author{中文论文稿 v30：phase-parity emitter 版}
\date{2026年7月8日}
...
\begin{abstract}
量子布尔函数 oracle 综合把经典谓词嵌入量子算法，是 Grover 搜索、量子密码分析和组合约束类量子程序中的基础编译环节。现有路线通常从固定布尔表示、可逆模板、LUT 映射、XAG 网络或 CNOT 导向空间结构出发；这些路线各自有效，但难以同时处理 T-count、CNOT、深度、门数和辅助比特之间的权衡。本文把 Boolean oracle synthesis 重新表述为代数正规形（algebraic normal form, \anf{}）和固定极性 Reed--Muller（fixed-polarity Reed--Muller, \fprm{}）项集合上的资源约束搜索问题，并提出 \method{}：一种结合神经动作先验、蒙特卡洛树搜索、布尔环线性因子筛选、结构深度策略、depth-frontier policy 与保守 guard 的逻辑层综合框架。本文方法不依赖 SSHR 的 parallelotope 候选空间；SSHR 在本文中只作为 small-scale CNOT-oriented baseline。

在 $n\leq6$ 的 177 个传统函数上，Pareto-\method{} 相对 direct \anf{} 平均 T-count 降低 72.25\%，平均 score 降低 67.80\%；相对 ESOP cube beam 获得 174/0/3 的 score 胜/负/平，平均 score 降低 36.09\%；相对 weighted ESOP-MILP 获得 167/3/7，平均 score 降低 29.84\%。在外部工具链方面，ROS-style LUT proxy 对 $n=3$--$6$ 与 $n=14,15,16,18$ 共 309 个函数产生 927 个 ABC $K=3,4,5$ 映射行，全部通过 \ttable{} 检查；best-$K$ proxy 相对固定 $K=4$ 平均 score 降低 18.12\%，但 Pareto-\method{} 相对该更强 proxy 仍为 309/0/0，平均 score 降低 84.27\%。进一步地，本文接入官方 mockturtle 头文件库，构建 ABC $K=4$ BLIF 到 mockturtle KLUT-to-XAG resynthesis 的可复现 probe；在传统 177 个函数上，Pareto-\method{} 相对该 probe 为 166/11/0，平均 score 降低 31.50\%；在 $n=14$ 的 64 个高维函数上为 64/0/0，平均 score 降低 91.49\%。RevKit Python \texttt{oracle\_synth} 在 $n\leq6$ 的 177 个函数上全部返回，但角度审计显示 171/177 行含非 Clifford \rz{}，总数为 9242；因此 RevKit lower-bound phase netlist 不能直接当作精确 Clifford+T T-count 对比。为把该边界从敏感性分析推进到实际 emitter，本文新增 phase-parity \anf{} baseline：将每个 \anf{} 单项式精确展开为 parity-phase gadgets，177/177 个函数均验证为 phase oracle（允许全局相位）。该 baseline 相对 RevKit lower-bound score 为 40/137/0、平均高 69.25\%，但每个非 Clifford \rz{} 加 1 个 score 单位后转为 177/0/0、平均降低 48.16\%，采用 $T/R_z=30$ 近似旋转综合 proxy 时为 177/0/0、平均降低 64.98\%。这说明 naive phase expansion 不是最终方法，但它给出了可验证的 phase/Rz-aware emitter 起点。

高维正确性通过两层桥接补强：在 $n=20$--$40$ 项集级 scale 中，6600/6600 个方法行通过 \anf{} plan 展开验证和 emitted X/CNOT/\mct{} circuit 的 GF(2) 多项式符号验证；在 $n=21,22,23$ truth-table bridge 中，300/300 个方法行通过完整 oracle 验证。阶段性结论是：\method{} 已经形成一条独立于 SSHR、以低 T-count 与低加权逻辑资源为目标的 Boolean oracle synthesis 主线；当前边界是主方法仍属于逻辑层 bit-flip oracle synthesis，phase-parity emitter 只是未优化的 phase/Rz 起点，尚未完成学习式 phase/Rz-aware search、官方 ROS 或 legacy RevKit/CirKit 完整复现，以及真实后端 mapping 评估。
\end{abstract}
...
本文研究的问题是：给定布尔函数 $f:\{0,1\}^n\rightarrow\{0,1\}$，如何自动生成实现
\begin{equation}
  |x\rangle |y\rangle \mapsto |x\rangle |y\xor f(x)\rangle
  \label{eq:oracle}
\end{equation}
的逻辑层可逆线路，并在 T-count、CNOT、深度、门数和辅助比特之间取得更好的资源权衡。本文不把问题简化为 CNOT-only optimization，也不把 AI 用作无约束门序列生成器。更准确的表述是：我们把 oracle synthesis 写成一个可验证的组合搜索问题，并让神经模型和 \mcts{} 帮助选择更有资源收益的代数分解动作。

本文的核心论证可压缩为一句话：

\begin{quote}
在逻辑层 bit-flip Boolean oracle synthesis 中，用 \anf{}/\fprm{} 项集合表示状态、用可反算的代数分解定义动作，并用资源感知神经搜索选择动作组合，可以在传统函数、高维项集合和若干外部逻辑网络 probe 上降低 T-count 与加权 score；该主张不覆盖 phase/Rz oracle、硬件 routing、magic-state factory、噪声模型或后端映射最优性。
\end{quote}

这个边界很重要。SSHR 的贡献在于利用 parallelotope 空间结构做 small-scale CNOT-oriented synthesis\cite{zheng2025sshr}，适合作为本文的 CNOT 导向 baseline；但本文方法不从 SSHR 候选空间出发，也不应把论文写成“SSHR 的小改进”。本文的对象是更一般的资源约束搜索，SSHR 只回答其中一个比较问题：当目标不是单纯 CNOT 最小时，\anf{}/\fprm{} 项集合搜索能否获得更低 T-count 和更低综合 score。
...
低 T-count oracle 综合长期依赖合适的布尔中间表示。Xor-And-Inverter Graphs（XAG）把 XOR 和 AND 结构分离，使 AND 节点数与非 Clifford 成本形成清晰联系。Meuli 等人讨论了 multiplicative complexity 在低 T-count oracle 编译中的作用\cite{meuli2019multiplicative}，并将 XAG 用于量子编译\cite{meuli2022xag}。ROS 将 oracle synthesis 写成 resource-constrained LUT mapping\cite{meuli2020ros}，进一步表明 LUT 分解、局部函数实现和辅助比特管理会共同影响资源。BDD-based reversible synthesis 则展示了面向大函数的符号结构路线\cite{wille2009bdd}。后端感知 oracle synthesis 开始把逻辑综合与 fault-tolerant back-end 指标连接起来\cite{yu2025backend}。

本文与这些工作的共同点是承认布尔中间表示决定量子逻辑资源。差异在于，本文不固定使用 XAG、LUT 或 BDD，而是以 \anf{}/\fprm{} 项集合为搜索状态。这样做牺牲了部分成熟工具链便利性，但换来两个直接优势：一是任意候选 plan 都能展开回 GF(2) 多项式进行等价检查；二是候选动作可以包含公共因子、极性重写、线性奇偶因子和布尔环共享变量结构，而不被单一网络表示限制。
...
本文的状态和动作与上述工作不同。我们不让模型直接输出门序列，而是让模型在可验证的代数动作之间排序或选择。最终线路仍由 compute/action/uncompute 结构生成，并经过 \anf{} 展开、GF(2) 多项式模拟或完整 \ttable{} 验证。这样的设计降低了 AI 产生错误线路的风险，也使论文的可复现性更多依赖确定性验证，而不是依赖模型表述。
...
\begin{equation}
  ...
  \label{eq:score}
\end{equation}
该分数只用于逻辑层候选排序，不代表真实硬件运行时间、物理错误率或 magic-state 工厂总成本。因此，实验同时报告 T、CNOT、深度和辅助比特，避免单一 score 掩盖 trade-off。

正确性验证分为三层。第一层在 $n\leq6$ 传统函数、$n=18,20$ 函数级切片和 $n=21,22,23$ bridge 中构造完整 \ttable{}，逐点验证式~\eqref{eq:oracle}。第二层在大规模项集实验中，将 factor plan 展开回 \anf{} 项集合，检查目标多项式是否一致。第三层对 emitted X/CNOT/\mct{} circuit 做 GF(2) 多项式符号模拟，检查输出线多项式、输入线复原和辅助线清零。第三层不是 $2^n$ 枚举，但它验证的是最终 emitted circuit，而不只是搜索 plan。

\section{方法}

\subsection{从项集合到可验证搜索}
...
Depth-frontier policy & 学习 depth-2/3/4 质量前沿，提供质量模式与 cost-aware 模式。 & 独立 seed 泛化和 $n=21,22,23$ bridge。 \\
...
传统函数切片包含 $n\leq6$ 的 177 个函数，baseline 包括 direct \anf{}、AND-direct \anf{}、ESOP cube beam、weighted ESOP-MILP、SSHR-H、ABC/BDD 估计、ROS-style LUT proxy、mockturtle KLUT-to-XAG probe 和 RevKit Python \texttt{oracle\_synth}。高维切片包含 $n=14,15,16,18$ 的外部导出函数、$n=18,20$ 函数级边界、$n=20$--$40$ 项集级 scale，以及 $n=21,22,23$ truth-table bridge。Paired comparison 只纳入函数名或项集名匹配、语义正确且未 timeout 的结果。Timeout 行作为可扩展性边界保留，但不进入资源均值。
...
表~\ref{tab:rz-boundary} 给出统一解释。若每个非 Clifford \rz{} 只加入 1 个 score 单位，Resource-NMCTS family 相对 RevKit 为 157/20/0；若加入 1.5 个单位，则达到 177/0/0。若采用 Ross--Selinger-style $T/R_z=30$ 的近似旋转综合 proxy，则 Resource-NMCTS family 为 177/0/0，平均 score 降低 95.03\%。这些结果说明 RevKit lower-bound 的优势主要来自未计入非 Clifford 旋转综合成本，但它们仍是成本模型，不是实际 rotation sequence 输出。
...
为了避免 phase/Rz 讨论只停留在 RevKit 侧的成本敏感性，本文进一步实现了一个内部 phase-parity \anf{} emitter。对每个非空 \anf{} 单项式 $S$，它使用整数恒等式
\begin{equation}
  \prod_{i\in S}x_i =
...
高维 RevKit 隔离探针进一步说明 API 可扩展性边界。对 $n=14$ 前 8 个函数逐行启动 RevKit \texttt{oracle\_synth} 子进程，并设置 30 s wall-time cutoff 后，仅 1 行返回，7 行 timeout。唯一返回函数 \texttt{anf\_n14\_10} 含 32767 个非 Clifford \rz{}。因此，高维 RevKit 目前只能作为 adapter/scalability boundary evidence，不能作为普通 paired resource benchmark 批量纳入均值。

\subsection{高维结构策略、验证桥接与 schedule proxy}
...
本文最稳妥的贡献表述是“资源约束搜索方法”，不是“AI 全面替代传统综合”。当前证据支持四点。第一，\anf{}/\fprm{} 项集合提供了清晰的语义验证基础，使搜索动作、线路生成和正确性检查可以统一在 GF(2) 多项式框架下。第二，Boolean-ring screen、depth-frontier policy 和 guard 使高维搜索不再只是扩大 beam，而是形成了结构候选、学习选择和保守兜底的组合。第三，外部 probe 显示本文方法不仅优于项目内部 baseline，也能在 ROS-style LUT proxy 和官方 mockturtle KLUT-to-XAG probe 下保持 score 优势。第四，phase-parity \anf{} emitter 把 RevKit/Rz 问题推进为一个实际可验证的内部 phase-oracle 生成器，虽然它还不是最终的优化方法。

同时，本文的局限必须在正文中正面承认。首先，SSHR-H 在平均 CNOT 上更低，说明本文不是 CNOT-only 方法。若目标场景极度 CNOT 受限，\method{} 的优势可能收窄。其次，RevKit lower-bound phase netlist 揭示了 gate-set 边界：本文已经补充了 phase-parity emitter，但它是朴素 parity expansion，lower-bound score 仍弱于 RevKit，尚未与主 \method{} 搜索统一，也没有输出实际 Clifford+T rotation sequence。再次，mockturtle probe 虽然使用官方头文件和 XAG NPN resynthesis，但仍不是完整 ROS，也没有 SAT garbage management 或可逆线路输出。最后，当前 schedule proxy 只是 emitted-circuit 层的逻辑调度代理，没有处理真实硬件连通性、routing、magic-state factory 和噪声模型。

因此，投稿前的“明显提升”目标应优先放在两个方向。第一，把 phase-parity emitter 从朴素展开升级为学习式或优化式 phase/Rz-aware search，并接入实际 approximate rotation synthesis 审计，使比较不只停留在 $T/R_z$ 常数 proxy。第二，继续复现官方 ROS 或 legacy RevKit/CirKit 完整流程，使外部对照从 adapter/probe 扩展到标准工具链。完成这两点后，本文就可以从“已有合理主线和阶段性证据”推进到“有标准外部对照、清晰 gate-set 边界和更强审稿说服力的论文”。
...
Phase/Rz 边界已有实际 emitter 起点 & RevKit 171/177 行含非 Clifford \rz{}；phase-parity \anf{} emitter 177/177 验证通过，score+1/Rz 为 177/0/0、$T/R_z=30$ proxy 为 177/0/0。 & 朴素 phase-parity lower-bound 只有 40/137/0；需要学习式 phase/Rz search 和实际旋转综合输出。 \\
高维结果具有语义可信度 & 6600/6600 项集级符号验证和 300/300 bridge 完整验证通过。 & $n=24$--$40$ 未做完整 \ttable{} 枚举。 \\
\bottomrule
\end{tabularx}
\end{table}
...
本文提出 \method{}，将量子布尔函数 oracle 综合建模为 \anf{}/\fprm{} 项集合上的资源约束搜索问题，并结合神经动作先验、\mcts{}、布尔环线性因子筛选、结构深度策略、depth-frontier policy 和保守 guard 生成逻辑层可逆线路。实验表明，该方法在传统函数上相对 ANF、ESOP、SSHR-H 和 weighted ESOP-MILP 获得稳定低 T-count 与低 score 优势，在 ROS-style LUT proxy 和官方 mockturtle KLUT-to-XAG probe 下仍保持优势，并通过项集级符号验证和 $n=21,22,23$ 完整 truth-table bridge 补强了高维正确性证据。

当前稿件已经形成独立于 SSHR 的论文主线：本文的贡献不是对 parallelotope SSHR 做局部改造，而是把 Boolean oracle synthesis 写成资源约束搜索，并让 AI 在可验证动作空间中承担搜索控制角色。v30 进一步补上了 phase-parity \anf{} emitter，使 RevKit/Rz 边界不再只是外部 baseline 的成本重解释。最终投稿仍需要把该 emitter 升级为学习式 phase/Rz-aware search，补强官方 ROS 或 legacy RevKit/CirKit 完整流程，以及真实后端相关评估。若这些缺口完成，本文的主张可以从阶段性方法证据推进为更强的、面向标准外部对照的资源约束 Boolean oracle synthesis 论文。
