...
2. phase/Rz 分支已经有 learned candidate-pruning 与 budget-frontier 诊断：top-512 在 93.75% 更少 exact scoring 下贴近 wide-128，但 same-budget random shortlist 的绝对差距仍小，说明当前 policy objective 仍需要加强；下一步需要从候选剪枝推进到 learned/optimized phase policy、旋转谱优化和 rotation-sequence audit，不能声明 phase/Rz 全局最优。
