...
20. Learned sparse depth-4 gate 把 sparse frontier 继续学习化：在 `n=16,20,24` 训练、`n=28,40` held-out 测试，保守阈值下 48 个测试 pair 中只运行 32 个 depth-4，false skip 为 0，相对 deterministic sparse depth-2/4 frontier score 为 0/0/48、平均 +0.00%，同时 sparse-frontier evaluation time -17.39%。该证据应写成搜索预算控制器，而不是比 deterministic sparse frontier 更强的质量前沿。
...
| learned sparse depth-4 gate | train `n=16,20,24`，held-out `n=28,40`，48 个测试 pair | vs sparse frontier 为 0/0/48、score +0.00%、false skip 0；只运行 32/48 个 depth-4，time -17.39% | 新增 sparse frontier 内部学习预算控制 |
