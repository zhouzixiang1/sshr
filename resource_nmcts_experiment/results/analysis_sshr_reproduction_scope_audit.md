...
| SSHR Table VIII candidate-space anchor | pass | source-anchored | The development repository contains the SSHR candidate-count reference and table runner when the full source tree is present; the upload payload records the boundary without requiring that parent source tree. | The paper can explain why SSHR's search space is a CNOT-oriented parallelotope counterpoint rather than the proposed ANF/FPRM action space. | This audit does not rerun every published SSHR random benchmark table inside the lightweight submission rebuild. |
