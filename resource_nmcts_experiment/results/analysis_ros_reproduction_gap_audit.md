...
| caterpillar/XAG source-family probe | pass | partial | The workstation has a local caterpillar checkout and source/build probe for XAG synthesis and SAT-based pebbling components; no standalone ROS executable is detected in that checkout. | The package now records a closer open-source implementation-family probe for XAG synthesis and quantum memory-management components. | caterpillar availability is not a full ROS SAT garbage-management reproduction and is not used as a new standalone performance baseline. |
