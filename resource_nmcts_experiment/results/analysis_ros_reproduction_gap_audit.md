...
| caterpillar/XAG source-family probe | pass | partial | The workstation has a local caterpillar checkout, API/build provenance, and a toy AIG compile/run smoke for logic-network synthesis and SAT/pebbling-related components; no standalone ROS executable is detected in that checkout. | The package now records a closer open-source implementation-family probe for XAG synthesis and quantum memory-management components. | caterpillar availability and a toy API smoke are not a full ROS SAT garbage-management reproduction and are not used as a new standalone performance baseline. |
