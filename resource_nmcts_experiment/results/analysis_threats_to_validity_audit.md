...
| AI/MCTS/RL attribution | pass | The manuscript could overstate deep learning, MCTS, or reinforcement learning as the sole reason for the gains. | Search-control and random-control audits separate the algebraic search space, MCTS, Pareto archives, and learned ranking; the fitted-Q budget controller uses disjoint train/validation/test functions and paired bootstrap intervals. | Reinforcement learning controls optional Pareto-search effort after a verified base result; it is not end-to-end deep RL and does not dominate always-on Pareto search in score. |
