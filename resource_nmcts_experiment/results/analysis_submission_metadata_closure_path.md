...
| Goal-closure gate consistency | pass | git_worktree=True; metadata_counts={'needs author input': 12}; author_input_closure_counts={'pass': 8}; goal_overall=not complete. | Do not mark the overall goal complete until author/venue metadata are filled and the readiness audit has no unresolved author-input rows. |
