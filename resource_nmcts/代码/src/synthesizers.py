...
# Locate models dir across layouts: reorg'd (代码/src -> ../../模型/models) or legacy (src -> ../models).
_SRC_PARENT = Path(__file__).resolve().parent.parent  # 代码/ in reorg'd layout, resource_nmcts/ in legacy
_MODELS_CANDIDATES = [
    _SRC_PARENT.parent / "模型" / "models",  # reorg'd: resource_nmcts/模型/models
    _SRC_PARENT / "models",                  # legacy: resource_nmcts/models
]
STRUCTURE_GATE_MODEL = next((p / "resource_structure_gate.json" for p in _MODELS_CANDIDATES if (p / "resource_structure_gate.json").exists()), _MODELS_CANDIDATES[1] / "resource_structure_gate.json")
