...
def add_learned_methods(
    ...
    for idx, ex in enumerate(examples):
        ...
        if frontier_depths is not None:
            frontier_method = f"screen_depth{frontier_depths[idx]}"
            if frontier_method in evals:
                chosen = evals[frontier_method]
...
def write_summary(summary_path: Path, analysis_path: Path, table_path: Path, examples: list[TermExample]) -> None:
    ...
    comparisons = [
        ...
        ("depth_frontier_policy", "screen_single"),
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    ap.add_argument("--policy-model", type=Path, default=THIS_DIR / "模型" / "models" / "boolean_screen_depth_policy.pt")
    ap.add_argument("--guard-model", type=Path, default=THIS_DIR / "模型" / "models" / "boolean_screen_depth_guard.pt")
    ap.add_argument(
        "--frontier-policy-model",
        type=Path,
        default=THIS_DIR / "模型" / "models" / "boolean_screen_depth_frontier_policy.pt",
    )
