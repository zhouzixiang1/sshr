...
The Benchmark function-diversity audit is generated by
`analyze_benchmark_function_diversity_audit.py` and writes
`results/analysis_benchmark_function_diversity_audit.md`; it records
function-diversity state for exact small-function families, density, degree,
ANF-term coverage, and large symbolic/bridge term-set profiles.
...
- `run_truth_bridge_terms.py --ns 24 --per-n 6 --tag truth_bridge_n24`,
  ...
  `run_truth_bridge_terms.py --ns 26 --per-n 6 --tag truth_bridge_n26`,
  `run_truth_bridge_terms.py --ns 27 --per-n 6 --tag truth_bridge_n27`,
  `run_truth_bridge_terms.py --ns 28 --per-n 6 --tag truth_bridge_n28`,
  `run_truth_bridge_terms.py --ns 29 --per-n 6 --tag truth_bridge_n29`, and
  `run_truth_bridge_terms.py --ns 30 --per-n 6 --tag truth_bridge_n30` extend
  the full truth-table bridge from `n=21,22,23` to `n=24,25,26,27,28,29,30`.
...
- `n=26`: 6 generated ANF functions and 60 method rows verify: 60/60 full
  oracle checks, 60/60 ANF plan checks, 60/60 emitted-circuit symbolic checks,
...
- `n=29`: 6 generated ANF functions and 60 method rows verify: 60/60 full
  oracle checks, 60/60 ANF plan checks, 60/60 emitted-circuit symbolic checks,
  mismatches=0.  Mean truth-table build time is 6.40 s per function; adaptive
  all-depth beats single screen by 6/0/0 and fixed depth-2 by 4/0/2, while the
  depth-frontier policy beats fixed depth-2 by 3/0/3 and remains close to
  depth-4/all-depth with less planning time.
- `n=30`: 6 generated ANF functions and 60 method rows verify: 60/60 full
  oracle checks, 60/60 ANF plan checks, 60/60 emitted-circuit symbolic checks,
  mismatches=0.  Mean truth-table build time is 27.69 s per function; adaptive
  all-depth beats single screen by 4/0/2 and fixed depth-2 by 4/0/2, while the
  depth-frontier policy beats fixed depth-2 by 3/0/3 and remains a bounded
  quality-time controller relative to depth-4/all-depth.
...
- This is a verification-boundary improvement, not a hardware-mapping claim;
  `n=31--64` remain covered by plan/circuit symbolic verification rather than
...
/opt/anaconda3/envs/mcts-qoracle/bin/python run_truth_bridge_terms.py --seed 20260719 --ns 28 --per-n 6 --workers 2 --action-width 6 --max-screen-depth 4 --tag truth_bridge_n28
/opt/anaconda3/envs/mcts-qoracle/bin/python run_truth_bridge_terms.py --seed 20260718 --ns 29 --per-n 6 --workers 1 --action-width 6 --max-screen-depth 4 --tag truth_bridge_n29
/opt/anaconda3/envs/mcts-qoracle/bin/python run_truth_bridge_terms.py --seed 20260720 --ns 30 --per-n 6 --workers 2 --action-width 6 --max-screen-depth 4 --tag truth_bridge_n30
