...
\author{中文论文稿 v35：Affine-FPRM phase search 版}
\date{2026年7月8日}
...
在 $n\leq6$ 的 177 个传统函数上，Pareto-\method{} 相对 direct \anf{} 平均 T-count 降低 72.25\%，平均 score 降低 67.80\%；相对 ESOP cube beam 获得 174/0/3 的 score 胜/负/平，平均 score 降低 36.09\%；相对 weighted ESOP-MILP 获得 167/3/7，平均 score 降低 29.84\%。在外部工具链 probe 中，ROS-style LUT proxy 覆盖 309 个函数和 927 个 ABC 映射行，best-$K$ proxy 相对固定 $K=4$ 平均 score 降低 18.12\%，但 Pareto-\method{} 相对该更强 proxy 仍为 309/0/0，平均 score 降低 84.27\%。官方 mockturtle KLUT-to-XAG probe 在传统 177 个函数上给出 166/11/0、平均 score 降低 31.50\%，在 $n=14$ 的 64 个高维函数上给出 64/0/0、平均 score 降低 91.49\%。高维正确性通过两层桥接补强：$n=20$--$40$ 项集级 6600/6600 个方法行通过 \anf{} plan 展开验证和 emitted circuit 符号验证；$n=21,22,23$ 基础 bridge 及两组 $n=23$ policy rerun 合计 280/280 个方法行通过完整 oracle 验证。本文进一步用位掩码 truth-table evaluator 将完整 bridge 推进到 $n=24,25$：新增两个 6 函数切片共 120/120 个方法行同时通过完整 oracle、plan \anf{} 和 emitted-circuit 符号验证，其中 $n=25$ 平均 truth-table 构造时间为 0.45 s/函数。至此本文累计报告 400/400 个高维 bridge 方法行的完整 oracle 验证。

本文同时报告 gate-set 边界。RevKit Python \texttt{oracle\_synth} 在 177 个函数上全部返回，但 171/177 行含非 Clifford \rz{}，总数为 9242，因此其 lower-bound phase netlist 不能直接当作精确 Clifford+T T-count 对比。为把该边界推进到项目内部，本文实现 phase-parity \anf{} emitter：将每个 \anf{} 单项式精确展开为 parity-phase gadgets，177/177 个函数均验证为 phase oracle up to global phase。该 emitter 相对 RevKit lower-bound score 为 40/137/0、平均高 69.25\%，但在每个非 Clifford \rz{} 加 1 个 score 单位后转为 177/0/0、平均降低 48.16\%，在 $T/R_z=30$ 近似旋转综合 proxy 下为 177/0/0、平均降低 64.98\%。进一步地，本文把 phase search 从 fixed-polarity \fprm{} 扩展到 Affine-FPRM：先在有界可逆线性变换 $B$ 下综合 $h(y)=f(B^{-1}y)$，再枚举极性 $p$ 得到 $g(z)=h(z\xor p)$，并把选中的 parity mask 精确翻译回原变量。Affine-FPRM 三个 rank metric 共 531 行全部验证通过；其中 $T/R_z=30$ 目标在 81/177 个函数选择非 identity 线性变换，相对 fixed-polarity FPRM 为 81/0/96、平均 score 降低 2.51\%，相对 phase-parity \anf{} 为 85/0/92、平均降低 2.98\%，相对 RevKit $T/R_z=30$ proxy 为 177/0/0、平均降低 65.50\%。这说明 phase/Rz 分支已经从成本敏感性和极性枚举推进到可验证的仿射代数预条件搜索；但它仍是逻辑层 phase-oracle emitter，不输出近似旋转序列，也不包含硬件 mapping。阶段性结论是：\method{} 已经形成一条独立于 SSHR、以低 T-count 与低加权逻辑资源为目标的 Boolean oracle synthesis 主线；当前缺口是更强的学习式 phase/Rz-aware search、官方 ROS 或 legacy RevKit/CirKit 完整复现，以及投稿级图表与论证收敛。
\end{abstract}
...
Truth-table bridge & 在 $n=21,22,23,24,25$ 构造完整 \ttable{}，把大规模项集符号验证和函数级完整验证连接起来。 \\
...
正确性验证分为三层。第一层在 $n\leq6$ 传统函数、$n=18,20$ 函数级切片和 $n=21,22,23,24,25$ bridge 中构造完整 \ttable{}，逐点验证式~\eqref{eq:oracle}。第二层在大规模项集实验中，将 factor plan 展开回 \anf{} 项集合，检查目标多项式是否一致。第三层对 emitted X/CNOT/\mct{} circuit 做 GF(2) 多项式符号模拟，检查输出线多项式、输入线复原和辅助线清零。第三层不是 $2^n$ 枚举，但验证的是最终 emitted circuit，而不只是搜索 plan。

为使 $n=24,25$ 完整枚举可运行，本文在验证 harness 中加入位掩码 truth-table evaluator。对变量 $x_i$ 预先构造赋值位集 $V_i$，单项式 $m=\prod_{i\in S}x_i$ 的真值表为
\begin{equation}
...
Depth-frontier policy & 学习 depth-2/3/4 质量前沿，提供质量模式与 cost-aware 模式。 & 独立 seed 泛化和 $n=21,22,23,24,25$ bridge。 \\
...
传统函数切片包含 $n\leq6$ 的 177 个函数，baseline 包括 direct \anf{}、AND-direct \anf{}、ESOP cube beam、weighted ESOP-MILP、SSHR-H、ABC/BDD 估计、ROS-style LUT proxy、mockturtle KLUT-to-XAG probe 和 RevKit Python \texttt{oracle\_synth}。高维切片包含 $n=14,15,16,18$ 的外部导出函数、$n=18,20$ 函数级边界、$n=20$--$40$ 项集级 scale，以及 $n=21,22,23,24,25$ truth-table bridge。Paired comparison 只纳入函数名或项集名匹配、语义正确且未 timeout 的结果。Timeout 行作为可扩展性边界保留，但不进入资源均值。
...
Truth-table bridge & $n=21,22,23$ 基础 bridge 及两组 $n=23$ policy rerun 合计 280 个方法行，加 $n=24,25$ 各 60 个方法行完整验证。 & 项集级符号验证和函数级 oracle 验证是否一致，以及完整验证边界能否继续上移。 \\
\bottomrule
\end{tabularx}
\end{table}
...
\input{../paper_latex/tables/mockturtle_xag_probe.tex}

\vspace{0.8em}
\input{../paper_latex/tables/mockturtle_xag_highdim_probe.tex}
\end{table}
...
\input{../paper_latex/tables/phase_parity_anf.tex}
\end{table}
...
\input{../paper_latex/tables/phase_parity_fprm.tex}
\end{table}
...
为进一步扩大 phase/Rz 搜索空间，本文新增 Affine-FPRM phase-parity search。它在 fixed-polarity 搜索前加入有界可逆线性预条件：对 GF(2) 可逆矩阵 $B$，先定义
\begin{equation}
  h(y)=f(B^{-1}y),\qquad g(z)=h(z\xor p),
\end{equation}
再对 $g$ 合成 phase polynomial。若该 polynomial 中某个 parity mask 为 $m_z$，则 $z=Bx\xor p$ 给出原变量上的 mask
\begin{equation}
  m_x=B^T m_z .
\end{equation}
当 $m_z\cdot p=1$ 时，对应 parity 被取反，因此旋转角取相反数并把原角度并入全局相位。这个过程只改变 phase polynomial 的代数表示，并不把 $B$ 当作额外硬件 CNOT wrapper 计入；验证仍逐输入检查 phase difference 是否为同一个有理数。搜索中包含 identity transform，因此在同一 rank metric 下 Affine-FPRM 相对 fixed-polarity FPRM 不退化。

\begin{table}[H]
\centering
\small
\caption{Affine-FPRM phase-parity search 的资源比较。每个函数搜索至多 32 个可逆线性变换，并在每个变换下枚举全部极性；共 531 行 selected results 全部 up-to-global-phase 验证通过。}
\label{tab:phase-parity-affine}
\input{../paper_latex/tables/phase_parity_affine.tex}
\end{table}

表~\ref{tab:phase-parity-affine} 显示，Affine-FPRM 是当前 phase/Rz 分支中比 fixed-polarity 更强的证据。在 $T/R_z=30$ 目标下，它有 81/177 个函数选择非 identity 线性变换、43/177 个函数选择非零极性；相对 fixed-polarity FPRM 为 81/0/96、平均 score 降低 2.51\%，相对 phase-parity \anf{} 为 85/0/92、平均降低 2.98\%，相对 RevKit 为 177/0/0、平均降低 65.50\%。按 lower-bound score 选择时，Affine-FPRM 相对 fixed-polarity FPRM 为 80/0/97、平均 score 降低 5.77\%。同时它仍不是完整 phase/Rz synthesis：当前报告的是 parity-gadget 级逻辑 emitter 与 $T/R_z$ proxy，不包含近似旋转序列生成、旋转综合运行时或硬件 mapping。
...
宽度消融进一步说明，单纯扩大 root action width 不是当前高维收益瓶颈。在 $n=20,28,40$、每个宽度 72 个项集的 probe 中，action-width 取 6、12、24 时每组 504/504 方法行都通过 plan 和 emitted-circuit 符号验证；adaptive all-depth 相对 fixed depth-2 始终为 0/0/72，depth-frontier policy 相对 all-depth adaptive 只高 0.08\% 左右，但随着宽度从 6 增至 24，全深度枚举相对 depth-2 的额外时间从 +57.52\% 降到 +43.04\%，而相对 single screen 的绝对开销从约 +3284\% 增至 +23918\%。这个结果支持本文默认使用 width 6 的结论：提升主要来自递归深度和 frontier 选择，而不是盲目加宽候选。

新增 $n=24$ 和 $n=25$ bridge 均使用式~\eqref{eq:truth-mask} 的位掩码 truth-table evaluator。$n=24$ 在 6 个生成式 \anf{} 函数上产生 60 个方法行，全部通过完整 \ttable{} oracle 验证、plan \anf{} 展开验证和 emitted-circuit 符号验证，mismatch 为 0。表~\ref{tab:n24-bridge} 显示，在该完整枚举切片上，adaptive all-depth 相对 single screen 为 6/0/0、平均 score 降低 9.42\%；相对 fixed depth-2 为 4/0/2、平均 score 降低 2.21\%。Depth-frontier policy 相对 fixed depth-2 为 3/0/3、平均 score 降低 2.05\%；相对 depth-4 只高 0.16\%，但 plan time 降低 27.08\%。

$n=25$ bridge 再次在 6 个生成式 \anf{} 函数上产生 60 个方法行，三层验证同样全部通过，平均 truth-table 构造时间为 0.45 s/函数。表~\ref{tab:n25-bridge} 显示，adaptive all-depth 相对 single screen 为 6/0/0、平均 score 降低 7.31\%；相对 fixed depth-2 为 4/0/2、平均 score 降低 2.14\%。Depth-frontier policy 相对 fixed depth-2 同样为 4/0/2、平均 score 降低 2.14\%；相对 depth-4 为 0/0/6，score 完全持平并节省 8.87\% plan time；相对 all-depth adaptive 也为 0/0/6，score 持平并节省 46.09\% plan time。该结果把完整验证边界从 $n=24$ 继续推到 $n=25$，同时表明 frontier policy 可以在更高维 bridge 上直接选择到质量前沿。
...
\input{../paper_latex/tables/truth_bridge_n24_terms.tex}
\end{table}

\begin{table}[H]
\centering
\scriptsize
\caption{$n=25$ 完整 truth-table bridge 的资源比较。}
\label{tab:n25-bridge}
\input{../paper_latex/tables/truth_bridge_n25_terms.tex}
\end{table}
...
$n=21,22,23$ 完整 \ttable{} oracle 验证 & 280/280 & 包含基础 bridge 及两组 $n=23$ policy rerun；所有 emitted circuit 逐点验证通过。 \\
$n=24$ 完整 \ttable{} oracle 验证 & 60/60 & 平均 truth-table 构造时间 0.18 s/函数；plan 与 emitted-circuit 符号验证也均为 60/60。 \\
$n=25$ 完整 \ttable{} oracle 验证 & 60/60 & 平均 truth-table 构造时间 0.45 s/函数；depth-frontier policy 与 depth-4 score 持平且 plan time 降低 8.87\%。 \\
...
但这还不是最终投稿形态。当前最强证据是逻辑层 bit-flip oracle synthesis 的低 T-count 与低 score；仍需继续补强的是 phase/Rz-aware search、标准外部工具链复现和投稿级结果组织。RevKit、phase-parity \anf{}、fixed-polarity phase search 与 Affine-FPRM 共同说明，gate-set 口径会显著改变结论。现在 phase/Rz 方向已经不只是成本敏感性分析：它有一个可验证 emitter、一个可验证极性搜索分支，以及一个可验证的仿射线性预条件分支；其中 Affine-FPRM 相对 fixed-polarity FPRM 已给出 81/0/96、平均 score 降低 2.51\% 的 $T/R_z=30$ proxy 增益。这个结果足以证明 phase/Rz 可以写成代数搜索问题，但仍不足以声明 phase/Rz 全局最优。$n=24,25$ bridge 则说明正确性验证能力可以继续上移，但它们仍是小样本完整枚举，不等同于 $n=26$--$40$ 全部完成 truth-table simulation。

因此，本文的“明显提升”目标应设置为三个可检查终点。第一，把 Affine-FPRM phase search 进一步扩展为学习式或优化式 phase/Rz-aware policy，在 177 个传统函数上保持不退化，并继续降低 parity gadget 数、非 Clifford \rz{} 数或综合后 proxy score，同时保持 selected rows 全部 up-to-global-phase 验证。第二，继续复现官方 ROS 或 legacy RevKit/CirKit 完整流程，使外部对照从 adapter/probe 扩展为标准工具链。第三，把当前 v35 的阶段性报告重组为投稿稿：围绕 4--6 张主图和若干补充表组织主张、baseline、公平性、消融和限制，而不是继续以实验日志形式累积结果。硬件 mapping 不属于本文的目标范围，本文只报告逻辑层资源和逻辑层 schedule/lifetime proxy。
...
Phase/Rz 边界已有实际 emitter 与 Affine-FPRM 搜索 & RevKit 171/177 行含非 Clifford \rz{}；phase-parity \anf{} emitter 177/177 验证通过；fixed-polarity 与 Affine-FPRM phase search 均 531/531 验证通过；Affine-FPRM 的 $T/R_z=30$ 目标相对 fixed-polarity FPRM 为 81/0/96、相对 \anf{} 为 85/0/92、相对 RevKit 为 177/0/0。 & 仍是 parity-gadget 级逻辑 emitter 与 proxy score；需要 learned phase/Rz policy 和实际旋转序列审计。 \\
高维结果具有语义可信度 & 6600/6600 项集级符号验证和 400/400 bridge 完整验证通过，其中 $n=24$ 与 $n=25$ 各为 60/60。 & $n=26$--$40$ 未做完整 \ttable{} 枚举。 \\
\bottomrule
\end{tabularx}
\end{table}
...
本文提出 \method{}，将量子布尔函数 oracle 综合建模为 \anf{}/\fprm{} 项集合上的资源约束搜索问题，并结合神经动作先验、\mcts{}、布尔环线性因子筛选、结构深度策略、depth-frontier policy 和保守 guard 生成逻辑层可逆线路。实验表明，该方法在传统函数上相对 ANF、ESOP、SSHR-H 和 weighted ESOP-MILP 获得稳定低 T-count 与低 score 优势，在 ROS-style LUT proxy 和官方 mockturtle KLUT-to-XAG probe 下仍保持优势，并通过项集级符号验证和 $n=21,22,23,24,25$ 完整 truth-table bridge 补强了高维正确性证据。

当前稿件最适合定位为“资源约束量子布尔函数综合的搜索方法论文”。它不是 SSHR 的附属改进，也不是泛泛使用 AI 做线路生成；它的核心贡献是把 oracle synthesis 的动作空间、搜索策略、资源模型和语义验证放在同一个 \anf{}/\fprm{} 框架中。v32 的 fixed-polarity phase-parity search 说明 phase/Rz 方向已经进入可验证搜索阶段，v35 的 Affine-FPRM 搜索进一步把该分支推进到线性预条件和极性的联合代数搜索；v33--v34 将高维完整 oracle 验证边界从 $n=24$ 推进到 $n=25$，并用 action-width probe 说明扩宽候选不是当前主收益来源。下一步若能把 learned phase/Rz policy、标准外部工具链复现和投稿级图表组织补齐，本文就可以从阶段性方法证据推进到更强的投稿版本。
