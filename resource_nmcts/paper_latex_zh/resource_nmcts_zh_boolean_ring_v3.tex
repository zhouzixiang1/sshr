...
\end{equation}
线路层面仍可先用 CNOT 计算线性奇偶量，再用该辅助量控制 quotient 子线路。因此，这一扩展不是单纯增加搜索宽度，而是把 Boolean algebra 中原本被实现约束排除的候选重新纳入资源搜索。对于 $n=20$ 这类 FPRM root beam 出现长尾的切片，本文进一步使用 recursive Boolean-ring screen：先以 direct plan 为 baseline 搜索 ANF-only Boolean-ring 因子，再在 quotient/rest 子问题上进行最多两层同类筛选，从而避免极性枚举长尾，同时保留 baseline-preserving 的候选选择。
...
$n=20$ 切片包含 6 个 random \anf{} 函数、8 个方法和 48 行结果，其中 12 行为 timeout。FPRM root beam 和 fast linear-pair 在所有 6 个函数上均触发 300 s task timeout。单层 ANF Boolean-ring screen 已经使 \method{} 相对 AND-direct ANF 获得 5/0/1，平均 score 降低 4.89\%。加入 depth-1 recursive Boolean-ring screen 后，集成 \method{} 相对旧 \method{} 的平均 score 降低 4.52\%；进一步加入 depth-2 screen 后，集成 \method{} 相对 depth-1 版本再降低 3.13\%，相对旧 \method{} 降低 7.47\%，相对 AND-direct ANF 降低 11.80\%。表~\ref{tab:n20} 给出核心比较。
...
\midrule
Depth-2 \method{} vs depth-1 \method{} & 6 & 5/0/1 & $-3.13\%$ \\
Depth-2 \method{} vs old \method{} & 6 & 5/0/1 & $-7.47\%$ \\
Depth-2 \method{} vs direct ANF & 6 & 5/1/0 & $-38.86\%$ \\
Depth-2 \method{} vs AND-direct ANF & 6 & 5/0/1 & $-11.80\%$ \\
Depth-2 Boolean screen vs depth-1 screen & 6 & 5/0/1 & $-3.13\%$ \\
FPRM root beam / fast linear-pair & 6 & 全部 timeout & 300 s task timeout \\
\bottomrule
\end{tabularx}
\end{table}

该结果更适合解释为边界可扩展性证据，而不是强神经搜索证据。$n=20$ 的收益主要来自递归布尔环 screen，说明某些高维 ANF 可以通过 Boolean algebra screen 获得改进；但该结果仍不能说明 learned prior 已经能在 $n=20$ 上做深搜索。depth-2 screen 的代价是平均运行时间上升到 20.15 s（standalone）/79.55 s（集成 \method{}），且 peak ancilla 略增。

\subsection{负面结果与不足}

负面结果同样重要。$n=18$ 的 ANF-only Boolean-linear screen 虽然平均只需 0.843 s，但相对旧 \method{} 为 0/12/0，平均 score 反而上升 42.45\%。recursive screen 相对单层 screen 获得 11/0/1，平均 score 降低 3.99\%，但相对旧 \method{} 仍为 0/11/1，平均 score 上升 36.94\%。这说明 screen 版本可以作为快速边界候选，不能替代 recursive Resource-NMCTS。

高维 learned prior 仍未达到理想强度。pairwise-wide root-action ranker 在 $n=16$ full synthesis 上相对 recursive guard 获得 10/0/14，平均 score 降低 0.18\%，属于可信小幅正向，但还不是“明显提升”。本轮 Boolean-linear root-teacher 模型接入 baseline-preserving guard 后，相对 deterministic Boolean-linear-deep 为 4/0/20，平均 score 降低 0.12\%，但平均运行时间增加 94.49\%。因此，下一阶段的 AI 贡献必须从动作重排转向结构选择：让模型预测何时使用 Boolean-ring factor、何时进入 FPRM 极性搜索、何时停止递归，以及如何在 CNOT/depth/ancilla trade-off 中选择候选。
...
本文表明，\method{} 在 \anf{}/\fprm{} 项集合上进行资源感知搜索，配合 Pareto portfolio、baseline-preserving guard 和布尔环线性因子，可以在传统函数和高维随机 \anf{} 上降低逻辑层 T-count 与加权 score。布尔环线性因子为 $n=16$ 提供了结构性 full-synthesis 改善；depth-2 recursive Boolean-ring screen 则在 $n=20$ 的 root-beam/fast-pair timeout 边界上把旧 \method{} 的平均 score 降低 7.47\%。不过，高维 learned prior 的贡献仍偏小，外部 reversible-toolchain 对比和线路案例仍需补齐。后续工作应围绕结构级神经策略、可复现实验矩阵和英文正式稿展开。
