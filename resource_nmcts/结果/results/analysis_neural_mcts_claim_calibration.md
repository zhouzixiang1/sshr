...
|---|---|---|---|---|
| Neural component in the title | pass | learned-control rows=13, promoted=5, bounded=6, limited=2, not_promoted=0; limited-boundary rows=6; effect-uncertainty rows=9 | Neural policies and learned gates are bounded ranking, pruning, or budget-allocation controls with promoted and limited evidence classes. | Do not claim that deep learning alone explains the resource reduction. |
| Reinforcement-learned budget control in the title | pass | budget-policy pairs=160, needs_revision=0 | A fitted-Q contextual bandit learns whether to spend the optional Pareto-MCTS budget and achieves a measured quality-effort tradeoff on disjoint test functions. | Do not describe the full synthesizer as end-to-end deep reinforcement learning or claim dominance over always-Pareto search. |
| MCTS component in the title | pass | search-control rows=13, needs_revision=0 | MCTS and Pareto search add measured, non-degrading increments over strengthened deterministic portfolios. | Do not attribute the full improvement to MCTS alone. |
...
| Verified workflow rather than learned correctness | pass | algorithm rows=8, budget rows=9 | Learning controls action order, pruning, or budget; correctness comes from GF(2), emitted-circuit, truth-table, and phase verifiers. | Do not imply that a neural model certifies circuit semantics. |
