...
from src.sshr_lib.baselines import esop_synthesize  # noqa: E402
from src.sshr_lib.sshr_h import sshr_h  # noqa: E402
from src.sshr_lib.sshr_beam import sshr_beam  # noqa: E402

from src.affine_search import affine_wrap_cost, candidate_transforms, emit_affine_wrapped, transform_function
...
from src.factor_plan import (
    ...
    greedy_plan,
    linear_pair_beam_plan,
    linear_pair_screen_plan,
    root_beam_plan,
    root_child_beam_plan,
    verify_circuit_anf,
    verify_oracle,
    verify_plan_anf,
)
...
from src.contracts.circuit import logical_ir_record, qasm_metadata_record
from src.contracts.codec import canonical_hex, sha256_file
from src.contracts.search import PlanTrace
from src.contracts.synthesis import DetailedSynthesisResult
from src.hardware.qasm import export_openqasm3


STRUCTURE_GATE_MODEL = Path(__file__).resolve().parent.parent / "models" / "resource_structure_gate.json"
FOUNDATION_MODEL = Path(__file__).resolve().parent.parent / "models" / "boolean_oracle_fm_v3.pt"
...
class DetailedResultUnavailable(RuntimeError):
    """Raised when a portfolio wrapper cannot expose one leaf-level trace."""


@dataclass
class _DetailedCapture:
    result: DetailedSynthesisResult | None = None
...
def _existing_circuit_cost(circ: QuantumCircuit, n_inputs: int) -> ResourceCost:
    T = CNOT = depth = 0
    peak = 0
    from src.sshr_lib.bool_func import mct_cost_rp
...
def _load_foundation_scorer(model_path: str | None):
    """Load the structured policy/value model used by ``foundation_nmcts``.

    Foundation checkpoints are deliberately kept out of ``_cached_scorer``:
    they do not implement the legacy 24-feature MLP format, and their action
    cache is valid only for the search configuration used by one solve.
    """
    from src.foundation.adapter import FoundationScorer

    path = Path(model_path).expanduser() if model_path else FOUNDATION_MODEL
    if not path.is_file():
        raise FileNotFoundError(
            f"foundation_nmcts checkpoint not found: {path}. "
            "Pass model_path explicitly or train models/boolean_oracle_fm_v3.pt."
        )
    return FoundationScorer.from_checkpoint(path)
...
def _solve_plan(method: str, terms: frozenset[int], config: SearchConfig, seed: int, neural):
    ...
    if method in {"neural_mcts", "fprm_neural_mcts"}:
        ...
        return solver.solve(terms)
    if method == "foundation_nmcts":
        if neural is None:
            raise ValueError("foundation_nmcts requires a FoundationScorer")
        ...
        solver = NeuralMCTSSolver(
            ...
            seed=seed,
            neural_scorer=neural,
            value_estimator=LearnedValueEstimator(neural, config),
        )
        return solver.solve(terms)
...
def _synthesize_impl(
    ...
    *,
    capture: _DetailedCapture | None = None,
) -> SynthesisResult:
    t0 = time.time()
    ...
    plan = None
    polarity = 0
    affine_rows: tuple[int, ...] | None = None
    if method == "foundation_nmcts":
        if config.gate_mode != "mct":
            raise ValueError(
                "foundation_nmcts currently supports gate_mode='mct' only: "
                "the bundled v3 checkpoint was trained in that cost domain. "
                "Train and identify a compatible checkpoint before using "
                "and_foundation_nmcts."
            )
        neural = _load_foundation_scorer(model_path)
    else:
    ...
    if method in {
        ...
        affine_rows = tuple(int(row) for row in transform.rows)
    ...
    if capture is not None:
        qasm_export = export_openqasm3(circ)
        factor_plan = plan if isinstance(plan, Plan) else None
        plan_verification = verify_plan_anf(factor_plan) if factor_plan is not None else None
        circuit_verification = verify_circuit_anf(circ, bf.n, original_terms)
        model_record = None
        if model_path:
            ...
            if resolved_model.is_file():
                ...
                try:
                    path_hint = resolved_model.relative_to(project_root).as_posix()
                except ValueError:
                    path_hint = resolved_model.name
                model_record = {
                    "model_id": resolved_model.stem,
                    "path_hint": path_hint,
                    "sha256": sha256_file(resolved_model),
                }
        elif method == "foundation_nmcts" and FOUNDATION_MODEL.is_file():
            model_record = {
                "model_id": FOUNDATION_MODEL.stem,
                "path_hint": FOUNDATION_MODEL.relative_to(
                    Path(__file__).resolve().parent.parent
                ).as_posix(),
                "sha256": sha256_file(FOUNDATION_MODEL),
            }

        transform_record = None
        if polarity or affine_rows is not None:
            transform_record = {
                "polarity_mask_hex": canonical_hex(int(polarity)),
                "affine_rows_hex": (
                    [canonical_hex(row) for row in affine_rows]
                    if affine_rows is not None
                    else None
                ),
            }
        capture.result = DetailedSynthesisResult(
            ...
            input={
                ...
                "anf_terms_hex": [canonical_hex(term) for term in sorted(original_terms)],
            },
            effective_config=asdict(config),
            model=model_record,
            summary=asdict(summary),
            plan_trace=PlanTrace.from_plan(factor_plan) if factor_plan is not None else None,
            plan_unavailable_reason=(
                None
                if factor_plan is not None
                else f"method {method!r} does not produce src.factor_plan.Plan"
            ),
            logical_ir=logical_ir_record(qasm_export.logical_ir),
            logical_qasm3=qasm_export.qasm,
            qasm_metadata=qasm_metadata_record(qasm_export.metadata),
            verification={
                "oracle_truth_table": {"ok": bool(correct)},
                "plan_anf": asdict(plan_verification) if plan_verification else None,
                "circuit_anf": asdict(circuit_verification),
            },
...
def synthesize(
    ...
    """Legacy summary entrypoint; its public signature and record stay stable."""

    return _synthesize_impl(method, bf, config, seed=seed, model_path=model_path)
...
def synthesize_detailed(
    ...
    """Run one leaf synthesizer and retain its Plan, logical IR and provenance.

    Portfolio and screen-gated wrappers return before a unique leaf Plan/circuit
    can be captured.  They therefore fail explicitly in v1 instead of emitting
    a misleading partial trace.  Their legacy :func:`synthesize` behavior is
    unchanged.
    """

    capture = _DetailedCapture()
    _synthesize_impl(
        method,
        bf,
        config,
    ...
    if capture.result is None:
        raise DetailedResultUnavailable(
            f"detailed result is unavailable for portfolio/wrapper method {method!r}; "
            "call a leaf synthesizer such as direct_anf, greedy_factor, "
            "mcts_factor, neural_mcts, or foundation_nmcts"
        )
