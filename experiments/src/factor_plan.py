...
def _apply_neural_prior(
    ...
    """Fold a scorer's opinion into each action's prior.

    Two scorer protocols are supported.  Legacy scorers expose
    ``score_many(features)`` and see only the flat ``action_features`` vector.
    Structural scorers expose ``score_actions`` and receive the raw term set,
    which is what a term-set encoder needs -- the hand-built feature vector has
    already discarded the structure by the time it is built.

    Dispatch is by capability, so every existing checkpoint keeps its exact
    previous behaviour.
    """
    if neural_scorer is None or not actions:
        return actions

    score_actions = getattr(neural_scorer, "score_actions", None)
    if score_actions is not None:
        scores = score_actions(
            terms, prefix_len, live_factor_ancilla, actions, direct_total, config
        )
    else:
        features = [
            action_features(
...
def candidate_actions(
    ...
    actions = _apply_neural_prior(
        actions, terms, prefix_len, live_factor_ancilla, direct_total, config, neural_scorer
    )
