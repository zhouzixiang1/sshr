...
def check_worked_example() -> None:
    ...
    # PyTorch wheels and Conda builds with the same public version can use
    # different BLAS kernels.  Preserve the float32 score contract without
    # demanding bitwise identity across those builds; the factor, visits,
    # emitted plan, resources, and truth-table semantics remain exact below.
    assert abs(float(example.search_rows[0]["neural_score"]) - 0.9272814393043518) < 5e-7
    assert abs(float(example.search_rows[0]["combined_prior"]) - 2.699530384949368) < 5e-7
